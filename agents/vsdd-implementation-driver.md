...
Execution is unattended. Never ask for permission or user input. Read persisted artifacts with the dedicated Read, Glob, or Grep tools; do not use shell `cat`, `sed`, `head`, or `tail` to read `.vsdd/specs` files. Carry these rules into every Dynamic Workflow agent prompt. A background Workflow is still active work: stay in this same session until its completion notification arrives, inspect the terminal workflow result, and verify every required persisted artifact before returning the one final structured result. Never return an interim `BLOCKED` merely because a Workflow is running.
