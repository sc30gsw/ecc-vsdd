...
- Set the candidate version consistently to the current release candidate (`1.0.0-rc.9` after authorizing the managed worktree via `--add-dir`).
...
Promote `v1.0.0-rc.9` to `v1.0.0` only when the same commit and installed payload have passed all gates.
