...
- Set the candidate version consistently to the current release candidate (`1.0.0-rc.10` after pairing the managed worktree with `acceptEdits`).
...
Promote `v1.0.0-rc.10` to `v1.0.0` only when the same commit and installed payload have passed all gates.
